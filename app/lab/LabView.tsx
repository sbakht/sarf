...
type Kind = Extract<WeaknessType, "ajwaf" | "mithal" | "naqis" | "mudaf" | "mahmuz_f">;
...
const DRILL_CELLS: { tense: Tense; person: PersonId; mood?: "indicative" | "jussive"; label: string }[] = [
...
export function LabView() {
  ...
  return (
    <div className="flex flex-col gap-6">
      <header>
        <p className="text-sm uppercase tracking-[0.2em] text-accent">Weak Verb Lab</p>
        <h1 className="mt-1 text-3xl font-semibold">Sound analog vs what you actually say</h1>
        <p className="mt-2 max-w-2xl text-ink-soft">
          Start from the regular template, then apply the mutation. أجوف is the most common; the other types use the same compare-and-drill layout.
        </p>
      </header>
        ...
        {KINDS.map((item) => (
          <button
            ...
            className={`rounded-full px-4 py-2 text-sm ${
              kind === item.id ? "bg-accent text-paper" : "border border-rule bg-card"
            }`}
          >
        ...
        <>
          <section className="rounded-3xl border border-rule bg-card p-6">
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div>
                ...
                <p className="mt-2 max-w-2xl leading-7 text-ink-soft">{meta.blurb}</p>
              </div>
              <div className="rounded-2xl bg-paper px-4 py-3">
                <p className="text-xs uppercase tracking-wider text-ink-soft">Example root</p>
                <p className="font-arabic text-2xl">
                  {root.letters.join(" ")}{" "}
                  <span className="text-base text-ink-soft">({root.gloss})</span>
                </p>
                {roots.length > 1 ? (
                  <button
                    type="button"
              ...
              <tbody>
                {rows.map((row) => (
                  <tr key={row.label} className="border-t border-rule align-top">
                    ...
                    <td className="p-3">
                      <ArabicWord slots={row.analog.slots} surface={row.analog.surface} />
                    </td>
                    <td className="p-3">
                      <ArabicWord slots={row.actual.slots} surface={row.actual.surface} />
                    </td>
                    <td className="p-3 text-left text-sm text-ink-soft">
                      {row.actual.mutations[0]?.rule ?? (row.actual.surface === row.analog.surface ? "No change in this cell." : "")}
                    </td>
                  </tr>
                ))}
              </tbody>
          ...
          {current ? (
            <section className="rounded-2xl border border-rule bg-card p-6">
              <p className="text-xs uppercase tracking-wider text-ink-soft">Drill</p>
              <h3 className="mt-1 text-xl font-semibold">
                What is the actual form for {current.label}?
              </h3>
              <p className="mt-1 text-sm text-ink-soft">
                Sound analog:{" "}
                <ArabicWord slots={current.analog.slots} surface={current.analog.surface} size="sm" />
              </p>
              <div className="mt-4 flex flex-wrap gap-2">
                {uniqueOptions.map((option) => {
                  const right = option === current.actual.surface;
                  const state =
                    picked == null ? "" : right ? "border-fa bg-fa/10" : option === picked ? "border-lam bg-lam/10" : "";
                  return (
                    <button
                      key={option}
                      type="button"
              ...
              {picked ? (
                <div className="mt-4 flex items-center gap-3">
                  <p className="text-sm text-ink-soft">
                    {picked === current.actual.surface ? "Correct." : `The actual form is ${current.actual.surface}.`}
                  </p>
                  <button
                    type="button"
